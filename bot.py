...
async def setup_container(container_id, status_msg, memory, username, vps_id=None, use_custom_image=False):
    ...
    try:
        ...
        hostname_cmd = f"echo 'unixnodes-{vps_id}' > /etc/hostname && hostname eaglenode-{vps_id}"
...
async def transfer_vps_command(ctx, vps_id: str, new_owner: discord.Member):
    ...
    try:
        ...
        await ctx.send(f"✅ EGALENODE VPS {vps_id} has been transferred from {ctx.author.name} to {new_owner.name}!")
